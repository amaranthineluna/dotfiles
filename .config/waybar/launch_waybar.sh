...
# current checksums
current_checksum_config=$(md5sum config.jsonc)
current_checksum_style=$(md5sum style.css)

# loop forever
while true; do
	# new checksums
	new_checksum_config=$(md5sum config.jsonc)
	new_checksum_style=$(md5sum style.css)
